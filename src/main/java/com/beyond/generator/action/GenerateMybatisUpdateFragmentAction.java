 ...
 * @date 2022/11/08
 */
public class GenerateMybatisUpdateFragmentAction extends GenerateMyBatisBaseAction {
    @Override
    ...
    private boolean genMapperXmlFragment(@NotNull Project project, PsiDocumentManager psiDocumentManager, PsiClass mapperClass, String methodName, PsiClass argumentClass, String argName, boolean isList) throws JDOMException, IOException {
        ...
        if (xmldoc != null) {
            ...
            Map<String, Object> whereMap = FragmentGenUtils.parseAndGenerateWhereMap(methodName);
            String where = (String) whereMap.get("where");
            List<Map<String,Object>> conditionMaps = (List<Map<String,Object>>) whereMap.get("conditions");
            String[] updateFields = StringUtils.splitByWholeSeparator(StringUtils.substringAfter(StringUtils.substringBefore(methodName, "By"), "update"), "And");
            Set<String> updateFieldSet = new HashSet<>();
            for (int i = 0; i < updateFields.length; i++) {
                updateFields[i] = StringUtil.deCapitalize(updateFields[i]);
                updateFieldSet.add(updateFields[i]);
            }
            ...
            Map<String, String> column2FieldName = new HashMap<>();
            for (PsiField allField : fields) {
                ...
                if (CollectionUtils.isNotEmpty(updateFieldSet)){
                    if (!updateFieldSet.contains(allField.getName())){
                        continue;
                    }
                }
                String column = StringUtil.humpToLine(allField.getName());
                column2FieldName.put(column, allField.getName());
            }

            if (isList){
                List<String> sets = new ArrayList<>();
                Map<String, Object> condition = conditionMaps.get(0);
                for (String column : column2FieldName.keySet()) {
                    String field = column2FieldName.get(column);
                ...
                for (String column : column2FieldName.keySet()) {
                    String fieldName = column2FieldName.get(column);
                    setFragments.add(String.format("%s=#{%s}", column, fieldName));
                }
                insertSql.append(String.join(",", setFragments));
                insertSql.append(" ");
                insertSql.append("\n where \n");
                insertSql.append(where);
            }
            String sql = "\n" + String.format("<update id=\"%s\">\n%s\n</update>", methodName, insertSql) + "\n";
    ...
    public @NotNull @IntentionFamilyName String getFamilyName() {
        return "generate mybatis fragment for update";
    }
