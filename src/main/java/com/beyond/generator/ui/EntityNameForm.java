 ...
 * @date 2021/02/22
 */
public class EntityNameForm extends DialogWrapper {
    ...
    public EntityNameForm(@Nullable Project project) {
        super(project);
    ...
    protected @Nullable JComponent createCenterPanel() {
        return form.addItem(new InputItem("packageName", "packageName: ", getProperty("entity.sqlgen.packageName", project), submitRunnable))
                .addItem(new InputItem("entityName", "entityName: ", getProperty("entity.sqlgen.entityName", project), submitRunnable))
                .buildPanel(2, 2);
    }
    ...
    public void show(Callback<EntityNameForm> submitCallback) {
    ...
    private class SubmitRunnable implements Runnable {
        ...
        private Callback<EntityNameForm> callback;

        @Override
        public void run() {
            saveFormProperties();
            if (callback != null) {
                callback.run(EntityNameForm.this);
            }
        }

        public Callback<EntityNameForm> getCallback() {
            return callback;
        }

        public void setCallback(Callback<EntityNameForm> callback) {
    ...
    private void saveFormProperties() {
        saveProperty("entity.sqlgen.packageName", getData().get("packageName"), true, project);
        saveProperty("entity.sqlgen.entityName", getData().get("entityName"), true, project);
    }
